...
local AbilityHandler = nil
local ClientDebounce = nil
local AbilityRemote = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("AbilityService"):WaitForChild("ToServer"):WaitForChild("AbilityActivated____")
...
local function getSpamTarget()
    local target = getClosestPlayerByWorld(80)
...
RunService.Heartbeat:Connect(function()
    if not spamEnabled or not AbilityHandler or not AbilityHandler.activeAbility then return end
...
local function enableRangeExpander()
    ...
    namecallHook = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        ...
        if spamEnabled and method == "InvokeServer" and self.Name == "RequestToReleasePlayer" then
            local targetToDrop = select(1, ...)
            task.spawn(function()
                if not AbilityHandler or not AbilityHandler.activeAbility then return end
                local ability = AbilityHandler.activeAbility
...
MiscTab:CreateToggle({
    ...
    Flag = "SpamMenuToggle",
    Callback = function(Value)
        local existing = game.CoreGui:FindFirstChild("WingsSpam")
        if Value then
            if existing then return end
            ...
            frame.Position = UDim2.new(0.5, -75, 0.5, 80)
            frame.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
            frame.BorderSizePixel = 0
            frame.Active = true
            frame.Draggable = true
            ...
            toggleBtn.MouseButton1Click:Connect(function()
                ...
                toggleBtn.Text = "SPAM: " .. (spamEnabled and "ON" or "OFF")
                toggleBtn.TextColor3 = spamEnabled and Color3.fromRGB(80, 255, 80) or Color3.fromRGB(255, 80, 80)
            end)
        else
            if existing then existing:Destroy() end
        end
...
MiscTab:CreateButton({
    ...
    Callback = function()
        ...
        pcall(disableESP)
        local spamGui = game.CoreGui:FindFirstChild("WingsSpam")
